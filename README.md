# Calculator
Project done under the The Odin Project's curriculum guidance.

## Key features
- Keyboard support and its visual help.
- Functional decimal poin (.) for both operands.
- Delete key.

## Learned
- Had to take a closer look at falsy values.
- Had to redo the main function from scratch.
- Understood how useful the developer tools are for debugging. There were some issues that were really complicated to spot without the tools.
- Tried using a custom event but it did not work out in the end.
- Got a bit more practice with the DOM.

## Fonts
- Font "Technology" by Vladimir Nykolic, at dafont.com.

## Reset file
- Reset file: from the Meyerweb website.
